...
def fetch(url: str, output_root: Path, *, metadata_only: bool = False, **options) -> dict:
    ...
    folder = output_root / f"xiaohongshu-{item_id}"
...
def _run_first_successful(commands: list[list[str]], *, timeout: int = 240) -> str:
    ...
    raise RuntimeError("yt-dlp Xiaohongshu route failed: " + " | ".join(errors))
...
def _require_ytdlp() -> str:
    ...
    if yt_dlp:
        return yt_dlp
    raise RuntimeError("Xiaohongshu provider requires yt-dlp, but yt-dlp is not installed or not on PATH.")
